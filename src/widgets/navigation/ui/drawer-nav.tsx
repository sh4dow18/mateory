    ...
    <Drawer MenuIcon={CiMenuBurger} inMobileOnly isNav>
      {ROUTES_LIST.map((route) => (
        <Link
          key={route.path}
          href={`/${route.path}`}
          className={`flex gap-3 p-3 ${route.isActive ? "bg-primary text-white rounded-md dark:bg-primary/50" : "text-gray-950 dark:text-gray-400 high-contrast:text-blackh"}`}
        >
